...
SheetService.getLastRow = function () {
  return SheetService.getSheet().getLastRow();
};
...
/**
 * Returns Number of Columns (safer when sheet empty)
 */
SheetService.getLastColumn = function () {
  var sheet = SheetService.getSheet();
  var last = sheet.getLastColumn();
  if (!last || last < 1) return Object.keys(CONFIG.HEADERS).length;
  return last;
};
...
SheetService.getTemplateRange = function () {
  return SheetService.getSheet().getRange(
...
function sheetServiceHealthCheck(){
  ...
  Logger.log("Sheet : " + SheetService.getSheet().getName());
  Logger.log("Template Row : " + SheetService.getTemplateRow());
...
SheetService.insertRows = function (startRow, numberOfRows) {
  ...
  SheetService.getSheet().insertRowsAfter(startRow - 1, numberOfRows);
};
...
SheetService.copyTemplateFormatting = function (targetRow, numberOfRows) {
  ...
  for (var i = 0; i < numberOfRows; i++) {
    template.copyTo(
      sheet.getRange(targetRow + i, 1, 1, SheetService.getLastColumn()),
...
SheetService.clearMerge = function (startRow, endRow) {
  ...
  SheetService.getSheet().getRange(startRow, participantsCol, endRow - startRow + 1, 1).breakApart();
  SheetService.getSheet().getRange(startRow, agendaCol, endRow - startRow + 1, 1).breakApart();
};
...
SheetService.mergeColumn = function (columnNumber, startRow, totalRows) {
  ...
  SheetService.getSheet().getRange(startRow, columnNumber, totalRows, 1).merge();
};
...
SheetService.mergeParticipants = function (startRow, totalRows) {
  ...
  SheetService.mergeColumn(participantsColumn, startRow, totalRows);
};
...
SheetService.mergeAgenda = function (startRow, totalRows) {
  ...
  SheetService.mergeColumn(agendaColumn, startRow, totalRows);
};
...
SheetService.mergeMeeting = function (startRow, discussionCount) {
  ...
  SheetService.mergeParticipants(startRow, discussionCount);
  SheetService.mergeAgenda(startRow, discussionCount);
};
...
SheetService.unmergeMeeting = function (startRow, discussionCount) {
  if (discussionCount < 1) discussionCount = 1;
  SheetService.clearMerge(startRow, startRow + discussionCount - 1);
};
...
SheetService.findMeetingRow = function (meetingCode) {
  ...
  var values = sheet.getRange(CONFIG.FIRST_DATA_ROW, meetingCodeColumn, lastRow - CONFIG.FIRST_DATA_ROW + 1, 1).getValues();
  for (var i = 0; i < values.length; i++) {
    if (String(values[i][0]).trim() === String(meetingCode).trim()) return CONFIG.FIRST_DATA_ROW + i;
  }
  return -1;
};
...
SheetService.getMeetingLastRow = function (meetingCode) {
  ...
  while (row < lastSheetRow) {
    var nextValue = sheet.getRange(row + 1, discussionColumn).getValue();
    var nextMeetingCode = sheet.getRange(row + 1, getColumn("MEETING_CODE")).getValue();
    if (nextMeetingCode !== "") break;
    if (nextValue === "") break;
    row++;
  }
  return row;
...
SheetService.findMeetingBlock = function (meetingCode) {
  ...
  var startRow = -1;
  var endRow = -1;
  var values = sheet.getRange(CONFIG.FIRST_DATA_ROW, codeColumn, lastRow - CONFIG.FIRST_DATA_ROW + 1, 1).getValues();
  for (var i = 0; i < values.length; i++) {
  ...
  if (startRow == -1) {
    return { found:false, startRow:-1, endRow:-1, rowCount:0 };
  }
  endRow = startRow;
  while (endRow < lastRow) {
    var nextMeetingCode = sheet.getRange(endRow + 1, codeColumn).getValue();
    if (String(nextMeetingCode).trim() != "") break;
    var discussion = sheet.getRange(endRow + 1, discussionColumn).getValue();
    if (String(discussion).trim() == "") break;
    endRow++;
  }
  return { found:true, startRow:startRow, endRow:endRow, rowCount:endRow-startRow+1 };
};
...
SheetService.getMeetingRowCount = function (meetingCode) {
  return SheetService.findMeetingBlock(meetingCode).rowCount;
};

SheetService.getMeetingStartRow = function (meetingCode){
  return SheetService.findMeetingBlock(meetingCode).startRow;
};

SheetService.getMeetingEndRow = function (meetingCode){
  return SheetService.findMeetingBlock(meetingCode).endRow;
};
...
SheetService.getColumnMap = function () {
  if (SheetService._columnCache !== null) return SheetService._columnCache;
